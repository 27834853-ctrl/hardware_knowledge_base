...
**performance-optimization.js 输出**:
```
📊 页面性能指标：
  - DOM 就绪: 814ms
  - DOM 内容加载: 826ms
...
**performance-monitor.js 输出**:
```
⏱️ Navigation Timing
┌──────────────────┬──────────────────────┐
│ domProcessing    │ -1770100844962       │ ❌ 异常负数
│ domContentLoaded │ -1770100844924       │ ❌ 异常负数
│ loadComplete     │ -1770100844924       │ ❌ 异常负数
│ totalTime        │ -1770100844927       │ ❌ 异常负数
└──────────────────┴──────────────────────┘
```
...
│ dnsLookup        │ 0            │ ✅ 正常
│ tcpConnection    │ 0            │ ✅ 正常
│ domProcessing    │ 等待中...    │ ✅ 友好提示
│ domContentLoaded │ 808          │ ✅ 正常
│ loadComplete     │ 等待中...    │ ✅ 友好提示
│ totalTime        │ 等待中...    │ ✅ 友好提示
└──────────────────┴──────────────┘
```
...
### 修改的文件
1. **performance-optimization.js** (Line 412-434)
   - 修复 `collectLoadMetrics()` 函数
   - 添加 `loadEventEnd` 有效性检查
   - 优化控制台输出

2. **performance-monitor.js** (Line 94-123, 509-527)
   - 修复 `collectNavigationTiming()` 函数
   - 添加 `domComplete`, `loadEventEnd` 有效性检查
   - 修复 `logPerformanceData()` 函数
   - 清理负数值，替换为"等待中..."提示
...
### 其他发现
- **CLS 值异常**: 控制台显示 `CLS: 1.0033`，远超阈值 0.1
  - 这表明页面存在明显的布局偏移
  - 可能原因：图片未设置尺寸、字体加载、动态内容插入
  - 优先级：中等（影响用户体验）
